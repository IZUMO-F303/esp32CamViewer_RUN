...
// Define the shared state for PWM channels
PwmChannelState pwm_states[MAX_PWM_CHANNELS];
...
static esp_err_t duty_handler(httpd_req_t *req)
{
    ...
    if (channel != -1 && channel < MAX_PWM_CHANNELS) {
        pwm_states[channel].duty = val;
    }
    ...
    if (!strcmp(variable, "led_intensity")) {
        ...
        if (!strcmp(variable, "framesize")) {
            ...
            if (val) {
                ...
                pwm_states[channel].enabled = true;
            } else {
                Serial.println("GPIO12 is now OFF");
                pwm_states[channel].enabled = false;
                ledcWrite(channel, 0); // Immediately turn off
            }
            res = 0;
        }
